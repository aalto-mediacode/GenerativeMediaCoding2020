...
void ofApp::draw(){
    ...
    ofSetColor(0, 255, 0);
    ofFill();
    ofDrawLine(0, 0, 1, waveform[1] * ofGetHeight()); //first line
    for(int i = 1; i < (ofGetWidth() - 1); ++i) {
        ofDrawLine(i, waveform[i] * ofGetHeight(), i + 1, waveform[i+1] * ofGetHeight());
    }
}
...
void ofApp::audioOut(ofSoundBuffer& output){
    ...
    for (int i = 0; i < output.getNumFrames(); ++i){
        double osc1 = oscillator1.square(mouseX)*(mouseY/float(ofGetHeight()));
        double osc2 = oscillator2.sinewave(mouseX)*(1-mouseY/float(ofGetHeight()));
        output[i * outChannels] = (osc1+osc2) * 0.5;
